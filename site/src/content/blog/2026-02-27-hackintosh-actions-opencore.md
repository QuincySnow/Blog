...
description: 分享一套无 Mac 环境下可用的工具链与操作顺序（镜像→U 盘→EFI→USB Map→安装）
...
1. 一般有两类工作流可以用：
   ...
   - 各版本对应的 **Build macOS \[版本名\]**（如 _Build macOS Sonoma_、_Build macOS Tahoe_ 等），相当于 README 里说的 Full Installer。
